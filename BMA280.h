...
#define DEAULT_ADDR_ACCL                    (0x18) //SDO tied to ground
...
/* @name: readRegister(address, reg, result)
 * @brief: read a single byte from the designated register, known issue with the eUSCI module that
 *          single reads are not handled properly. TODO: fix the misread, requires double read to get
 *          accurate value
